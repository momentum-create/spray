...
import { inboundToShopProduct } from "@/content/inbound/shop-product";

export function GentemCollectionView() {
  return (
      ...
      <section id="products" className="scroll-mt-24 mt-8">
        ...
        <div className="mt-6 grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
          {inboundProducts.map((product) => (
            <ProductCard key={product.slug} product={inboundToShopProduct(product)} />
          ))}
        </div>
      </section>
